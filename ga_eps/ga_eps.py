...
from ga_eps import optimizers
from ga_eps import policies
from ga_eps import utils
from ga_eps import tf_util
    ...
    def _init(self):
        ...
        for p in range(self.config["pop_size"]):
            with tf.Graph().as_default():
                ...
                self.policy = policies.GenericPolicy(self.sess, env, env2, env.action_space, env.observation_space,
                                                     preprocessor, self.config["observation_filter"],
                                                     self.config["model"], **policy_params)
        ...
        while num_episodes < min_episodes or num_timesteps < min_timesteps:
            logger.info(
                "Iteration {}, collected {} episodes {} timesteps so far this iter, cur_par={},rew_max={}, rew_mean={}, rew_min={}, last rew_max1={}, rew_mean1={},rew_max2={}, rew_mean2={},rew_max3={}, rew_mean3={},rew_max4={}, rew_mean4={}".format(
                    self.iteration + 1, num_episodes, num_timesteps, self.curr_parent, reward_max, reward_mean,
                    reward_min, self.noisy_rew_max1, self.noisy_rew_mean1, self.noisy_rew_max2, self.noisy_rew_mean2,
                    self.noisy_rew_max3, self.noisy_rew_mean3, self.noisy_rew_max4, self.noisy_rew_mean4))
            rollout_ids = [
    ...
    def _train(self):
        ...
        if population2.shape[0] >= config["population_size"]:
            if len(returns2_n2.tolist()) >= config["population_size"]:
                idx = np.argpartition(returns2_n2, (-config["population_size"], -1))[
                      -1:-config["population_size"] - 1:-1]
